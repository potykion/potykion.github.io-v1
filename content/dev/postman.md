...
[Postman](https://www.postman.com/)(ака постман) - десктопная прога для отправки запросиков на серв

## Зач это надо

- чтобы протестить что серв/апи отдает нормальные данные
- чтобы иметь общий язык с серверным разрабом
  - Напр. когда ты мобильный разраб, кидаешь запросики и получаешь ошибку, можно чекнуть Postman на предмет правильности
    отправки запросов
- делится апишкой своего серва с кем угодно
...
1. Качаем с офф-сайта, ставим, логинимся видим начальный экран - Launchpad, жмем **Create a request**
...
4. И для закрепа кинем GET-запрос с query-параметрами:
...
Эту строку передаем в другие запросики способом, определенным разработчиком
...
Basic Auth - когда сразу кидаем запросы, имея логин и пароль, не кидая при этом дополнительных запросов
...
Здесь Variable - это название переменной, Initial Value - можно оставить пустым, Current Value - значение переменной, которое будет отправляться вместе с запросом
...
Для коллекции можно выставить переменные только единоразово, а иногда хочется иметь возможность иметь наборы переменных, между которыми можно быстро переключаться (напр. разные доступы в рамках одной апишки)
...
Работает и настраивается все так же как и для коллекций, но приоритет у енвайронментов выше. Так что если выставить в коллекции переменную `token`, и в енвайронменте, то значение будет взято из енва
...
Как я писал выше, вместо того чтобы писать руками в каждый запрос токен, можно использовать выставить его в переменные 

Но часто бывает так, что токен имеет ограниченный срок годности и его нужно обновлять, а каждый раз копировать токен из запроса и проставлять его в коллекции или енве может надоесть.

Чтобы каждый раз ручками не обновлять токен, нужно написать немножко кода, и в Postman как раз есть возможность запускать код после отправки запроса
...
Для того чтобы взаимодействовать с Postman из кода, нужно использовать объект `pm` - в нем есть доступ к ответу на запрос, а так же возможность выставлять переменные
...
В данном случае считаем, что в ответе на запрос приходи json такого вида: `{"token": "какой-то токен"}`, мы читаем его и выставляем в переменные коллекции
...
- [hoppscotch](https://hoppscotch.io/) - браузерная альтернатива
